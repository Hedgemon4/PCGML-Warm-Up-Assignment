...
# Hyperparameters
totalEpisodes = 100  # total number of episodes to run for (likely way too few, just here as a starting point, try 100 or 1000 for more consistency)
...
learningRate = 0.05  # how quickly to update the q-values (increase for faster convergence, decrease for more consistent learning/optimization)
discountFactor = 0.9  # how much to pay attention to future changes (increase for more longterm behaviour)
...
for i in range(0, totalEpisodes):
    ...
    for j in range(len(SARs) - 1, 0, -1):
        ...
        newQValue = oldQValue + learningRate * (
            SARs[j][2] + discountFactor * optimalFutureValue - oldQValue
        )
